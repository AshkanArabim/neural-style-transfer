...
def style_transfer(configs):
    ...
    optimizer = torch.optim.Adam([output_image], lr = 1)
    for i in range(configs["max_iters"]):
        ...
        variance_loss, content_loss, style_loss, loss = utils.calculate_loss(
            output_image, out_style_reps, out_content_rep, style_reps, content_rep, configs
        )
        # print("iteration:", i, "\tloss:", loss)
        print(f"iter {i}/{configs['max_iters']} | loss: {loss}")
